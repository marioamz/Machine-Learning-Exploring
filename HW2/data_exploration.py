...
def read_data(csv_file, plot_type, var1, var2):
    '''
    ...
    explore_data(credit)
    clean_df = clean_data(credit)
    plot_data(clean_df, plot_type, var1, var2)
    
    # This had to be checked again since Nan values were introduced in discretizing
    clean_df['monthly_income_groups'].fillna(0, inplace=True)
    clean_df['age_groups'].fillna(0, inplace=True)
    
    # Delete one last useless variable
    del clean_df['PersonID']
